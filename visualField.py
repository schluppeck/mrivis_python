...
fixSpot = visual.PatchStim(myWin, tex="none", mask='gauss', pos=(
    0, 0), size=(0.5, 0.5), color=[-1.0, -1.0, -1.0])
...
while carryOn:  # quits after 20 secs
    ...
    status.setText('x=%.1f, y=%.1f, r=%.1f' %
                   (stimPos[0], stimPos[1], stimSize[0]/2))
...
if saveDlg.OK:
    misc.toFile('visualFieldParams.pickle', {
                'size': surr.size[0], 'centre_x': surr.pos[0], 'centre_y': surr.pos[1]})
